...
use fedimint_gateway_v1::events::compute_lnv1_stats;
use fedimint_gateway_v1::pay::{OutgoingPaymentError, OutgoingPaymentErrorType};
use fedimint_gateway_v1::{GatewayClientModule, GatewayExtPayStates, LightningV1Manager};
...
#[async_trait]
impl LightningV1Manager for Gateway {
    ...
    async fn check_pruned_invoice(&self, payment_data: PaymentData) -> anyhow::Result<()> {
        let lightning_context = self.get_lightning_context().await?;
