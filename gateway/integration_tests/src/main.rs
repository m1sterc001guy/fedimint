...
async fn liquidity_test() -> anyhow::Result<()> {
    devimint::run_devfed_test()
        .call(|dev_fed, _process_mgr| async move {
            ...
            for gw in gateways {
                let txid = gw
                    .send_onchain(dev_fed.bitcoind().await?, BitcoinAmountOrAll::Amount(fedimint_core::bitcoin::Amount::from_sat(100_000)), 10)
                    .await?;
                bitcoind.poll_get_transaction(txid).await?;
            }

            // Verify sending all onchain creates a transaction
            let txid = gw_ldk_second
