...
pub trait LightningClientExt {
    ...
    ) -> anyhow::Result<UpdateStreamOrOutcome<'_, InternalExtPayState>>;

    async fn subscribe_lightning_pay(
        &self,
        operation_id: OperationId,
...
async fn check_internal_payment(invoice: &Invoice, gateways: Vec<LightningGateway>) -> bool {
...
impl LightningClientExt for Client {
    ...
    async fn pay_bolt11_invoice(&self, invoice: Invoice) -> anyhow::Result<(PayType, ContractId)> {
        ...
        let gateways = self.fetch_registered_gateways().await?;
        let (pay_type, output, contract_id) = if check_internal_payment(&invoice, gateways).await {
    ...
    async fn subscribe_internal_pay(
        ...
        Ok(operation.outcome_or_updates(self.db(), operation_id, || {
            stream! {
                yield InternalExtPayState::Funding;
                ...
                let state = loop {
                    if let Some(LightningClientStateMachines::InternalPay(state)) = stream.next().await {
                        match state.state {
                            InternalPayStates::Preimage(preimage) => break InternalExtPayState::Preimage(preimage),
                            InternalPayStates::RefundSubmitted(txid) => {
                                let out_point = OutPoint { txid, out_idx: 0};
                                match self.await_primary_module_output(operation_id, out_point).await {
                                    Ok(_) => break InternalExtPayState::RefundSuccess(out_point),
                                    Err(e) => break InternalExtPayState::RefundError(e.to_string()),
                                }
                            },
                            InternalPayStates::FundingFailed(e) => break InternalExtPayState::FundingFailed(e),
                            _ => {}
                        }
                    }
                };
...
impl ClientModule for LightningClientModule {
    ...
    fn context(&self) -> Self::ModuleStateMachineContext {
        // FIXME: Manage this key in a secure way
        let mut rng = rand::rngs::OsRng;
        let ctx = secp256k1::Secp256k1::new();
        let redeem_key = KeyPair::new(&ctx, &mut rng);
...
impl LightningClientModule {
    ...
    /// gateway
    pub async fn create_incoming_output<'a, 'b>(
        &'a self,
        operation_id: OperationId,
        invoice: Invoice,
    ) -> anyhow::Result<(
        ...
        let invoice_amount = Amount {
            msats: invoice
                .amount_milli_satoshis()
                .ok_or(InternalPayError::AmountError)?,
        };
...
pub enum LightningClientStateMachines {
    InternalPay(InternalPayStateMachine),
