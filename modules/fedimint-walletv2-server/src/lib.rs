    ...
    async fn process_block_count(
        ...
        for height in old_consensus_block_count..new_consensus_block_count {
            ...
            for tx in block.txdata {
                ...
                // We maintain an append-only log of valid P2TR transaction outputs created
    ...
    fn sign_tx(&self, unsigned_tx: &FederationTx) -> Vec<Signature> {
        ...
        unsigned_tx
            ...
            .map(|(index, utxo)| {
                ...
                secp256k1::SECP256K1.sign_schnorr(&secp256k1::Message::from(sighash), &keypair)
            })
            .collect()
    }
