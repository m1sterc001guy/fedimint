...
use fedimint_ln_client::{
    InternalExtPayState, LightningClientExt, LightningClientGen, LnReceiveState, PayType,
};
...
async fn makes_internal_payments_within_federation() -> anyhow::Result<()> {
    ...
    match pay_type {
        PayType::Internal(op_id) => {
            ...
            assert_eq!(sub2.ok().await?, InternalExtPayState::Funding);
            assert_matches!(sub2.ok().await?, InternalExtPayState::Preimage { .. });
