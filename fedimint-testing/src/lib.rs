    ...
    Module::Decoder: Sync + Send + 'static,
{
    ...
    pub async fn verify_input(&self, input: &Module::Input) -> Result<TestInputMeta, ModuleError> {
        ...
        for (_, member, db, module_instance_id) in &self.members {
            ...
            results.push(
                member_validate(
                    member,
                    &mut dbtx.with_module_prefix(module_instance_id.clone()),
                    &fake_ic,
                    input,
                )
                .await,
                ...
                member
                    .validate_output(
                        &mut db
                            .begin_transaction()
                            .await
                            .with_module_prefix(module_instance_id.clone()),
                        output,
                    )
                    .await
                    .is_err(),
        ...
        <Module as ServerModule>::Input: Send + Sync,
    {
        ...
        for (_peer, member, db, module_instance_id) in &mut self.members {
            ...
            {
                let mut module_dbtx = dbtx.with_module_prefix(module_instance_id.clone());
